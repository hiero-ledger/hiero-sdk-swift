...
internal final class TopicMessageSubmitTransactionTests: XCTestCase {
    ...
    internal func testSetCustomFeeLimits() throws {
        let customFeeLimits = [
            CustomFeeLimit(
                ...
                customFees: [
                    AnyCustomFee.fixed(FixedFee(amount: 1, denominatingTokenId: TokenId("0.0.1")))
                ]),
            CustomFeeLimit(
                payerId: AccountId("0.0.2"),
                customFees: [
                    AnyCustomFee.fixed(FixedFee(amount: 1, denominatingTokenId: TokenId("0.0.2")))
                ]),
        ]
    ...
    internal func testAddCustomFeeLimitToList() throws {
        ...
        let customFeeLimitToAdd = CustomFeeLimit(
            ...
            customFees: [
                AnyCustomFee.fixed(FixedFee(amount: 3, denominatingTokenId: TokenId("0.0.3")))
            ])
