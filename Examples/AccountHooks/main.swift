...
internal enum Program {
    internal static func main() async throws {
        ...
        let hookCreationDetails2 = HookCreationDetails(
            ...
            adminKey: Key.single(contractKey.publicKey))
        let updateReceipt = try await AccountUpdateTransaction()
            .accountId(accountId)
        ...
        let deleteReceipt = try await AccountUpdateTransaction()
            .accountId(accountId)
            .addHookToDelete(1)
            .execute(client)
            .getReceipt(client)
        ...
        try await cleanup(client, accountId: accountId, contractId: contractId, operatorAccountId: env.operatorAccountId)
    ...
    private static func createHookContract(_ client: Client) async throws -> (ContractId, PrivateKey) {
        ...
        let bytecode = ""
        let contractKey = PrivateKey.generateEd25519()
        let contractId = try await ContractCreateTransaction()
            .bytecode(dataFromHex(bytecode))
