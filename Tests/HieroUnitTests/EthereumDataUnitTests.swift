...
public final class EthereumDataUnitTests: HieroUnitTestCase {
    internal func test_LegacyFromBytes() throws {
    ...
    internal func test_LegacyRoundtrip() throws {
        let data = try EthereumData(rlpBytes: rawTxType0)

        XCTAssertEqual(rawTxType0.hexStringEncoded(), data.toBytes().hexStringEncoded())
    }

    internal func test_Eip1559FromBytes() throws {
    ...
    internal func test_Eip1559Roundtrip() throws {
        let data = try EthereumData(rlpBytes: rawTxType2)
