...
internal class TopicCreate: XCTestCase {
    ...
    internal func testChargesHbarFeeWithLimitsApplied() async throws {
        ...
        // Create a topic with custom fees enabled
        let receipt = try await TopicCreateTransaction()
            .adminKey(.single(testEnv.operator.privateKey.publicKey))
            .feeScheduleKey(.single(testEnv.operator.privateKey.publicKey))
        ...
        _ = try await TopicMessageSubmitTransaction()
            .topicId(topicId)
            .message(Data("Hedera Hbar Fee Test".utf8))
            .execute(testEnv.client)
            .getReceipt(testEnv.client)
    ...
    internal func testExemptsFeeExemptKeysFromHbarFees() async throws {
        ...
        _ = try await TopicMessageSubmitTransaction()
            .topicId(topicId)
            .message(Data("Hedera Fee Exemption Test".utf8))
            .execute(testEnv.client)
            .getReceipt(testEnv.client)
    ...
    // operator's account ID if it is not set
