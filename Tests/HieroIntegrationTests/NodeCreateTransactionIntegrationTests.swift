...
internal class NodeCreateTransactionIntegrationTests: HieroIntegrationTestCase {
    ...
    internal func test_CreateNewNetworkNode() async throws {
        // Given
        ...
        let endpoint = Endpoint(ipAddress: nil, port: 1234, domainName: "1234.com")
        let endpoint1 = Endpoint(ipAddress: nil, port: 5678, domainName: "5678.com")
        let validGossipCert = Data(hexEncoded: Self.validGossipCertDerHex)!
        let adminKey = PrivateKey.generateEd25519()
        let grpcProxyEndpoint = Endpoint(ipAddress: nil, port: 12345, domainName: "testWeb.com")

        // When
        let txReceipt = try await NodeCreateTransaction()
            .accountId(accountId)
            .description(description)
            .gossipEndpoints([endpoint, endpoint1])
            .serviceEndpoints([endpoint, endpoint1])
